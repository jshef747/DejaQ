...
def list_keys(workspace_slug: str):
    try:
        return admin_service.list_keys(workspace_slug)
...
def generate_key(
    ...
    try:
        return admin_service.generate_key(workspace_slug, force=force)
    except admin_service.WorkspaceNotFound as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def delete_revoked_key(key_id: int):
    try:
        return admin_service.delete_revoked_key(key_id)
    except admin_service.KeyNotFound as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
