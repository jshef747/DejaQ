...
| `app/services/` | The pipeline: `normalizer`, `context_enricher`, `context_adjuster`, `validator`, `cache_filter`, `classifier`, `lexical_match`, `memory_chromaDB` (semantic cache), `image_fingerprint` + `image_text` (image gate), `file_text` (PDF/Markdown file gate). Plus infra: `model_backends` (OllamaBackend + `MODEL_RUNTIME_SPECS`), `service_factory`, `llm_router`, `external_llm` + `llm_providers/` (Google/OpenAI/Anthropic), `credential_service`, `provider_inference`, `admin_service`, `stats_service`, `llm_config_service`, `feedback_service`, `request_logger` |
...
File caching on `/v1/responses` (one PDF or Markdown file per request): identity is an exact sha256 of the extracted text, so there are no thresholds, no eval harness and no false merges; files without a readable text layer (scanned PDFs) are refused rather than guessed at. PDFs reach the provider as a native document part, Markdown is inlined behind a data delimiter.
