...
| `DEJAQ_GENERALIZE_NGRAM_REPEAT_RATIO_MAX` | `0.08` | Store-time safety net: floor of the maximum word 4-gram repetition ratio allowed in a generalized rewrite; above the ceiling the output is a loop and the raw answer is stored instead. The ceiling scales with the raw answer's own repetition, so a faithful rewrite of a legitimately templated answer (a 50-item numbered list measures 0.35-0.48) is not read as a loop (`app/config.py`) |
...
| `DEJAQ_ADJUST_NGRAM_REPEAT_RATIO_MAX` | `0.08` | Serve-time safety net: floor of the maximum word 4-gram repetition ratio allowed in a tone adjustment; above the ceiling the output is a loop and the cached answer is served verbatim. This is the signal the topic-overlap check is blind to - a loop paraphrasing the cached answer scores near-perfect overlap. Scales with the cached answer's own repetition, since the adjust prompt requires every bullet and numbered item to survive (`app/config.py`) |
