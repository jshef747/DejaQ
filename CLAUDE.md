...
| `DEJAQ_GENERALIZE_NGRAM_REPEAT_RATIO_MAX` | `0.08` | Store-time safety net: maximum word 4-gram repetition ratio allowed in a generalized rewrite; above it the output is a loop and the raw answer is stored instead. Skipped when the rewrite kept the raw answer's own size (see `DEJAQ_NGRAM_EXEMPT_LENGTH_RATIO`) (`app/config.py`) |
