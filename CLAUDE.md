...
| `DEJAQ_OLLAMA_NUM_CTX` | `32768` | Context window sent with every Ollama request. Bounds the prompt as well as the generation, so it must hold `DEJAQ_REWRITE_MAX_TOKENS` of output plus the answer being rewritten; left unset Ollama uses its own much smaller default and silently drops the head of an oversized prompt. 32768 is the smaller model's (qwen2.5:1.5b) own maximum, so it is safe on both (`app/config.py`) |
...
| `DEJAQ_NGRAM_EXEMPT_LENGTH_RATIO` | `1.3` | How far a rewrite's length may differ from the answer it was rewriting, in either direction, and still skip the two repetition ceilings above. Exists for one population: a faithful rewrite of an answer that is *already* repetitive (a templated list, a weekly schedule), which both prompts require to keep every item and which therefore inherits its source's repetition without inventing any. A loop leaves the band by growing (measured 2.27x and up) or by collapsing onto one item (0.56x), and then faces the unmodified ceiling (`app/config.py`) |
