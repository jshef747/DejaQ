...
| `DEJAQ_ADJUST_LENGTH_ABS_FLOOR` | `200.0` | Characters below which the adjust length ratio is not applied, so a legitimate ELI5 or "explain in detail" rewrite of a one-line cached answer isn't rejected purely by its tiny denominator (`app/config.py`) |
