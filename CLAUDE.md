...
`/admin/v1/*` is a separate operator surface from the OpenAI-compatible `/v1/*` gateway. Its authentication follows `config.AUTH_MODE` (`config.py:242-245`): **`local` by default** — an unauthenticated dev-admin context — whenever `SUPABASE_URL` is blank, and Supabase JWT validation otherwise. In `local` mode the surface is protected by loopback binding, not by a credential. See [Management auth modes](#management-auth-modes-adminv1) above.
