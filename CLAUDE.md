...
| `DEJAQ_ADJUST_NGRAM_REPEAT_RATIO_MAX` | `0.08` | Serve-time safety net: maximum word 4-gram repetition ratio in a tone adjustment; above it the output is a loop and the cached answer is served verbatim. This is the signal the topic-overlap check is blind to - a loop paraphrasing the cached answer scores near-perfect overlap (`app/config.py`) |
