...
| `DEJAQ_OLLAMA_NUM_CTX` | `32768` | Context window sent by the two rewrite roles only (generalize, adjust) - every other role keeps Ollama's own default, since a window costs a KV cache per loaded model and no other role comes near it. Bounds the prompt as well as the generation, so it must hold `DEJAQ_REWRITE_MAX_TOKENS` of output plus the answer being rewritten; left unset Ollama silently drops the head of an oversized prompt. 32768 is the smaller model's (qwen2.5:1.5b) own maximum, so it is safe on both (`app/config.py`) |
