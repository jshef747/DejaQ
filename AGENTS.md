...
## A large inlined-text attachment can still 400 raw on the external path

`DEJAQ_MAX_ATTACHMENT_BYTES` (10MB) only bounds what DejaQ accepts; it says nothing about the *external provider's* context window once a text/Markdown/code attachment (not PDF - those go as a native document part) is inlined into the prompt. A ~10.48MB text file, just under the byte cap, was judged too large for `local_attachment_max_tokens` and routed external, where Groq rejected the resulting oversized prompt with the same opaque `"...may not be supported"` message the attachment-capability gate (see above) was built to replace for the *modality* case - there is no equivalent check for the *token-budget* case. Measured on `dejaq-acceptance-retest/report.md`. Not fixed; needs a pre-flight token-count check against the configured external model's context window, or a truncation/rejection policy for inlined-text attachments specifically.

## Generic pronoun-only follow-ups can cross-collide between unrelated conversations

The enricher's standalone-question rewrite sometimes translates a Hebrew follow-up ("how long does this take?") into a *generic English* standalone question, losing the conversation-specific referent (which dish, which topic). Two completely unrelated conversations whose follow-ups both enrich into near-identical generic English phrasing become reachable by the same cache entry at band-tier distance, and the validator - which only sees the new question against the cached answer, not which conversation either came from - approves the mismatch. One live instance: a lentil-soup conversation served an unrelated cached answer about Carbonara pasta prep time, further garbled by the context adjuster drifting to English on top. Measured on `dejaq-acceptance-retest/report.md` (`conversation_shapes` probe). Not fixed - `needs-decision`: whether to extend the "never translate" instruction (already on the generalizer/adjuster prompts) to the enricher's prompt, add conversation-scoped signal to the validator/lookup, or accept this as a known false-merge surface for the eviction/feedback loop to age out.
