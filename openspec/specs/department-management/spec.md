...
The system SHALL use a department's `cache_namespace` (e.g. `"acme-corp__customer-support"`) as the ChromaDB collection name when reading or writing cache entries for requests scoped to that department. The anonymous fallback namespace `"dejaq_default"` and the per-workspace default `"{workspace_slug}--default"` SHALL likewise resolve to ChromaDB collections of the same name. Collections are created on first use via `get_or_create_collection`.
...
- **WHEN** a cached entry exists in `"acme-corp--default"` for query Q
