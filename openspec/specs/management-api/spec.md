...
- `GET /admin/v1/orgs` — list every organization as `[{id, name, slug, created_at}]`.
- `POST /admin/v1/orgs` — create org from `{name}`; return HTTP 201 with the new org. Duplicate slug SHALL return HTTP 409.
- `DELETE /admin/v1/orgs/{slug}` — delete an org and cascade departments, API keys, and LLM config; return HTTP 200 with `{"deleted": true, "departments_removed": <int>}`. Unknown slug SHALL return HTTP 404.

#### Scenario: List orgs

- **WHEN** a client calls `GET /admin/v1/orgs`
- **THEN** the response is HTTP 200 with a JSON array of every organization

#### Scenario: CLI lists the same orgs

- **WHEN** the CLI calls the org listing service
...
- **WHEN** a client calls `DELETE /admin/v1/orgs/does-not-exist`
- **THEN** the response is HTTP 404

#### Scenario: Delete org cascades departments

- **WHEN** a client deletes an org with three departments
...
- `GET /admin/v1/departments?org=<slug>` — list departments; `org` query param optional. Without `org`, return departments across every organization. With `org`, return departments for that org. Each item SHALL include `{id, org_slug, name, slug, cache_namespace, created_at}`.
...
- **WHEN** a client calls `GET /admin/v1/departments?org=acme`
...
- **THEN** departments across every org are returned, each carrying its `org_slug`

#### Scenario: List departments for unknown org

- **WHEN** a client calls `GET /admin/v1/departments?org=does-not-exist`
- **THEN** the response is HTTP 200 with an empty array

#### Scenario: Create department under unknown org

- **WHEN** a client posts to `/admin/v1/orgs/missing/departments`
- **THEN** the response is HTTP 404
...
- `GET /admin/v1/orgs/{org_slug}/keys` — list keys as `[{id, token_prefix, created_at, revoked_at}]`. The `token_prefix` SHALL be the first 12 chars followed by `...`; the full token SHALL NOT be returned. Unknown org → HTTP 404.
- `POST /admin/v1/orgs/{org_slug}/keys?force=<bool>` — generate a new key. Without `force`, an existing active key SHALL cause HTTP 409. With `force=true`, the existing active key SHALL be revoked first. The response (HTTP 201) SHALL include the full token (one-time visibility). Unknown org → HTTP 404.
...
- **WHEN** a client calls `GET /admin/v1/orgs/acme/keys`
- **THEN** each entry contains `token_prefix` only and never the full secret

#### Scenario: List keys for unknown org

- **WHEN** a client calls `GET /admin/v1/orgs/does-not-exist/keys`
- **THEN** the response is HTTP 404
...
- `GET /admin/v1/stats/orgs?from=<ISO-8601 date>&to=<ISO-8601 date>` — totals per org plus a grand total across all orgs.
...
`GET /admin/v1/stats/orgs` SHALL return `{items: OrgStats[], total: StatsMetrics}`. `OrgStats` SHALL include `{org, org_name, requests, hits, misses, hit_rate, avg_latency_ms, est_tokens_saved, easy_count, hard_count, models_used}`. `StatsMetrics` SHALL include the aggregate metric fields without identity fields, aggregated over every org row.

`GET /admin/v1/stats/orgs/{org_slug}/departments` SHALL return `{org, items: DepartmentStats[], total: StatsMetrics}`. `DepartmentStats` SHALL include `{org, department, department_name, requests, hits, misses, hit_rate, avg_latency_ms, est_tokens_saved, easy_count, hard_count, models_used}`. Unknown org SHALL return HTTP 404.

#### Scenario: Per-org stats with no date filter

- **WHEN** a client calls `GET /admin/v1/stats/orgs`
- **THEN** the response is HTTP 200 with one row per org plus a `total` field aggregating every row

#### Scenario: Per-department stats for a single org

- **WHEN** a client calls `GET /admin/v1/stats/orgs/acme/departments`
- **THEN** the response contains one row per department under `acme`

#### Scenario: Stats include every org

- **WHEN** requests exist for both `acme` and `globex`
- **THEN** `GET /admin/v1/stats/orgs` includes both `acme` and `globex` stats

#### Scenario: Stats empty when no requests were logged

- **WHEN** a client calls `GET /admin/v1/stats/orgs` against an empty request log
...
- **WHEN** a client calls `GET /admin/v1/stats/orgs?from=2026-04-01&to=2026-04-15`
...
- **WHEN** a client calls `GET /admin/v1/stats/orgs/missing/departments`
- **THEN** the response is HTTP 404

### Requirement: Org LLM config endpoints

The system SHALL expose org LLM configuration endpoints:
- `GET /admin/v1/orgs/{org_slug}/llm-config` — return routing configuration. Unknown org SHALL return HTTP 404.
- `PUT /admin/v1/orgs/{org_slug}/llm-config` — update routing configuration. Unknown org SHALL return HTTP 404. Validation errors SHALL return HTTP 422.

#### Scenario: Read LLM config

- **WHEN** a client calls `GET /admin/v1/orgs/acme/llm-config`
- **THEN** the response is HTTP 200 with the org's LLM routing configuration

#### Scenario: Update LLM config with an invalid value

- **WHEN** a client calls `PUT /admin/v1/orgs/acme/llm-config` with a value outside the allowed range
- **THEN** the response is HTTP 422

#### Scenario: LLM config for unknown org

- **WHEN** a client calls `GET /admin/v1/orgs/missing/llm-config`
- **THEN** the response is HTTP 404

### Requirement: Feedback management endpoints

The system SHALL expose feedback management endpoints:
- `GET /admin/v1/feedback` — list feedback entries across every organization, optionally filtered by `workspace`, `department`, or `response_id`.
- `POST /admin/v1/feedback` — create or record management feedback. Unknown org or response metadata SHALL return HTTP 404. Validation errors SHALL return HTTP 422.

Collection responses SHALL include resources from every organization.

#### Scenario: List feedback includes every org

- **WHEN** feedback exists for both `acme` and `globex`
- **THEN** `GET /admin/v1/feedback` includes feedback associated with both

#### Scenario: Create feedback for unknown org

- **WHEN** a client posts feedback naming an org that does not exist
- **THEN** the response is HTTP 404
