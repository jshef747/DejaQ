...
```bash
DEJAQ_EXTERNAL_MODEL=Codex-haiku-4-5-20251001 DEJAQ_START_LOGS=requests ./start.sh --stack=server --mode=local &
```
...
(`anthropic` when using `Codex-haiku-4-5-20251001`). Without an encrypted workspace credential
...
> "Script is ready. Run this in your terminal — Codex can't run it due to tool timeout limits:
> ```bash
